...
class WebRTCService {
  ...
  public onParticipantVideoToggle?: (participantId: string, enabled: boolean) => void;
  public onParticipantAudioToggle?: (participantId: string, enabled: boolean) => void;
  public onParticipantScreenShare?: (participantId: string, enabled: boolean) => void;
  public onParticipantHandRaised?: (participantId: string, raised: boolean, name: string) => void;
  public onNewMessage?: (message: ChatMessage) => void;
  public onMeetingJoined?: (participants: Participant[]) => void;
  public onJoinRequest?: (request: {id: string, name: string, email?: string}) => void;
  ...
  private setupSocketListeners() {
    ...
    this.socket.on('participant-hand-raised', ({ participantId, raised, participantName }) => {
  ...
  async joinMeeting(meetingId: string, userInfo: { name: string; email?: string; isHost?: boolean }) {
    ...
    try {
      ...
      console.log('🎥 [WebRTC] Media constraints:', { video: true, audio: true });
      ...
      console.log('🎥 [WebRTC] Stream tracks:', this.localStream.getTracks().map(t => ({
        kind: t.kind,
        enabled: t.enabled,
  ...
  toggleVideo(enabled: boolean) {
    if (this.localStream && this.socket) {
      ...
      if (videoTrack) {
        ...
        this.socket.emit('toggle-video', { meetingId: this.meetingId, enabled });
      }
    }
  }
  ...
  toggleAudio(enabled: boolean) {
    if (this.localStream && this.socket) {
      ...
      if (audioTrack) {
        ...
        this.socket.emit('toggle-audio', { meetingId: this.meetingId, enabled });
      }
    }
  }
  ...
  async startScreenShare() {
    try {
      ...
      if (this.socket) {
        this.socket.emit('toggle-screen-share', { meetingId: this.meetingId, enabled: true });
      }
  ...
  async stopScreenShare() {
    ...
    if (this.socket) {
      this.socket.emit('toggle-screen-share', { meetingId: this.meetingId, enabled: false });
    }
  }
  ...
  requestToJoin(meetingId: string, userInfo: { name: string; email?: string; isHost?: boolean }) {
    if (this.socket) {
  ...
  getLocalStream() {
    console.log('🔍 [WebRTC] Getting local stream:', this.localStream ? 'Available' : 'Not available');
    if (this.localStream) {
  ...
  isLocalStreamReady() {
    const isReady = this.localStream !== null && 
