import { pool, connectDB } from "../src/config/database";
...
const seedData = async () => {
  try {
    ...
    await pool.execute(
      ...
      ["admin@example.com", adminPassword, "System", "Admin", "admin"]
    );
    ...
    await pool.execute(
      "INSERT OR IGNORE INTO users (email, password, first_name, last_name, role) VALUES (?, ?, ?, ?, ?)",
      ["user@example.com", userPassword, "John", "Doe", "user"]
    );
    ...
    for (const skill of skills) {
      await pool.execute(
        "INSERT OR IGNORE INTO skills (name, description, category) VALUES (?, ?, ?)",
        [skill.name, skill.description, skill.category]
      );
    }

    // Get skill IDs for creating questions
    const [skillRows] = await pool.execute("SELECT id, name FROM skills");
    const skillMap = new Map();
    (skillRows as any[]).forEach((skill) => {
    ...
    if (skillMap.has("JavaScript")) {
      for (const question of jsQuestions) {
        await pool.execute(
          "INSERT OR IGNORE INTO questions (skill_id, question_text, option_a, option_b, option_c, option_d, correct_answer, difficulty) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
          [
