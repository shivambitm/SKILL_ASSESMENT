...
const seedData = async () => {
  try {
    ...
    for (const skill of skills) {
      await pool.query(
        "INSERT INTO skills (name, description, category) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING",
        [skill.name, skill.description, skill.category]
      );
    }

    // Get skill IDs for creating questions
    const skillResult = await pool.query("SELECT id, name FROM skills");
    const skillMap = new Map();
    skillResult.rows.forEach((skill: any) => {
    ...
    if (skillMap.has("JavaScript")) {
      for (const question of jsQuestions) {
        await pool.query(
          "INSERT INTO questions (skill_id, question_text, option_a, option_b, option_c, option_d, correct_answer, difficulty) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)",
          [
